...
PTrade提供了丰富的财务数据API，支持基本面分析和量化选股。以下是详细的API说明：
...
**1. valuation - 估值数据**
- 支持字段：total_value(总市值)、pb(市净率)、pe_dynamic(动态市盈率)、turnover_rate(换手率)等
- 注意：只支持按天查询，不支持年份范围查询
- 换手率和股息率返回带%的字符串，需要手动转换

**2. balance_statement - 资产负债表**
- 支持字段：total_assets(资产总计)、total_liability(负债合计)、cash_equivalents(货币资金)等

**3. income_statement - 利润表**
- 支持字段：net_profit(净利润)、operating_revenue(营业收入)、basic_eps(基本每股收益)等

**4. cashflow_statement - 现金流量表**
- 支持字段：net_operate_cash_flow(经营活动现金流)、invest_cash_paid(投资支付现金)等

**5. growth_ability - 成长能力指标**
- 支持字段：operating_revenue_grow_rate(营收增长率)、net_profit_grow_rate(净利润增长率)等

**6. profit_ability - 盈利能力指标**
- 支持字段：roe(净资产收益率)、roa(总资产净利率)、gross_income_ratio(毛利率)等

**7. eps - 每股指标**
- 支持字段：basic_eps(基本每股收益)、naps(每股净资产)、net_operate_cash_flow_ps(每股经营现金流)等

**8. operating_ability - 营运能力指标**
- 支持字段：inventory_turnover_rate(存货周转率)、total_asset_turnover_rate(总资产周转率)等

**9. debt_paying_ability - 偿债能力指标**
- 支持字段：current_ratio(流动比率)、quick_ratio(速动比率)、debt_equity_ratio(产权比率)等
...
valuation_data = get_fundamentals(stocks, 'valuation', fields=['pb', 'pe_dynamic', 'total_value'])

# 获取净资产收益率
roe_data = get_fundamentals('600570.SS', 'profit_ability', fields='roe', date='2023-12-31')

# 获取资产负债表数据（按年份查询）
balance_data = get_fundamentals('600570.SS', 'balance_statement',
                               fields='total_assets',
                               start_year='2020', end_year='2023',
                               report_types='1')

# 获取利润表数据（按日期查询）
income_data = get_fundamentals('600570.SS', 'income_statement',
                              fields='net_profit',
                              date='20231231')
```
...
1. **日期参数说明**：
   - 不传date：回测中获取context.blotter.current_dt的数据，可能产生未来函数
   - 传入date：获取指定日期的数据，建议使用前一交易日避免未来函数

2. **数据格式**：
   - 换手率(turnover_rate)和股息率(dividend_ratio)返回带%的字符串
   - 需要手动转换：`float(turnover_rate.replace('%', '')) / 100`

3. **查询模式**：
   - 按日期查询：返回指定日期前最新的财务数据
   - 按年份查询：返回指定年份范围内的季度数据

4. **数据可用性**：
   - 估值数据：每日更新
   - 财务报表数据：按季度发布，以公告日期为准

#### 在策略中的应用

```python
def initialize(context):
    g.stocks = get_index_stocks('000906.XBHS')  # 中证800成分股
    set_universe(g.stocks)

def before_trading_start(context, data):
    # 获取股票池的估值数据
    valuation = get_fundamentals(g.stocks, 'valuation',
                                fields=['pb', 'pe_dynamic'],
                                date=get_trading_day(-1))

    # 获取盈利能力数据
    profitability = get_fundamentals(g.stocks, 'profit_ability',
                                   fields=['roe', 'roa'],
                                   date=get_trading_day(-1))

    # 基本面选股：选择ROE>15%且PE<30的股票
    good_stocks = []
    for stock in g.stocks:
        if (stock in profitability.index and stock in valuation.index):
            roe = profitability.loc[stock, 'roe']
            pe = valuation.loc[stock, 'pe_dynamic']
            if roe > 15 and pe < 30:
                good_stocks.append(stock)

    g.selected_stocks = good_stocks[:20]  # 选择前20只

def handle_data(context, data):
    # 基于基本面选股结果进行交易
    for stock in g.selected_stocks:
        order_target_percent(stock, 1.0/len(g.selected_stocks))
```

这些财务数据API为量化策略提供了强大的基本面分析能力，可以与技术分析相结合，构建更加全面的投资策略。